...
function calculate_price(tier, durability) {
    ...
    const final_price = base_price * tier * (durability / 30);
...
const MembershipTable = () => {
    ...
    return (
        <table className={styles.table}>
            <thead>
                <tr>
                    <th className={styles.header}>Tier / Duration</th>
                    {defaultDurabilities.map(durability => <th key={durability} className={styles.header}>{durability} days</th>)}
            ...
            <tbody>
                {tiers.concat(["Custom"]).map((tier, index) => (
                    <tr key={index}>
                        <td className={styles.cell}>{tier === "Custom" ? (
                            <input
                                type="number"
                                value={customTier}
                        ...
                        {defaultDurabilities.concat(customDurability).map((durability, idx) => (
                            <td key={idx} className={styles.cell}>
                                {durability ? `$${calculate_price(tier === "Custom" ? customTier : index + 1, durability)}` : '-'}
                            </td>
                        ))}
                    </tr>
                ))}
